...
export async function getAll(req, res, next) {
  try {
    ...
    const status = queryStringFirst(req.query.status);
    const from = queryStringFirst(req.query.from);
    const to = queryStringFirst(req.query.to);
    const items = await nearMissService.findAllNearMisses(req.qhseTenantId, {
      siteId,
      status,
      from,
      to
...
export async function create(req, res, next) {
  try {
    const row = await nearMissService.createNearMiss(req.qhseTenantId, req.body ?? {});
...
export async function update(req, res, next) {
  try {
    ...
    const row = await nearMissService.updateNearMiss(req.qhseTenantId, id, req.body ?? {});
    res.json(row);
  } catch (err) {
    if (err.statusCode === 400) {
      return res.status(400).json({ error: err.message });
...
export async function remove(req, res, next) {
  try {
    const id = typeof req.params.id === 'string' ? req.params.id.trim() : '';
    await nearMissService.deleteNearMiss(req.qhseTenantId, id);
    res.status(204).send();
  } catch (err) {
    if (err.statusCode === 400) {
      return res.status(400).json({ error: err.message });
